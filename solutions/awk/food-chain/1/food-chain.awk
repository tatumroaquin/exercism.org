...
function get_verse(num, anim, map, j) {
	verse = ""
	verse = verse sprintf("I know an old lady who swallowed a %s.\n", anim[num])
	verse = verse map[anim[num]] "\n"

	for (j = num; j > 1 && num != 8; j--) {
		if (j == 3) {
			spider = substr(map["spider"], 4, length(map["spider"]))
			verse = verse sprintf("She swallowed the %s to catch the %s that %s\n", anim[j], anim[j-1], spider)
			continue
		}
